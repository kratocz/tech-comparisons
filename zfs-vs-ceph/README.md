...
- **Facts verified:** July 2026 · addenda 2026-08-01/06 (§2.5–2.6 the Linux snapshot layer; §15 reliability profiles and the corruption-bug timelines) · 2026-08-13 (§16 growing one disk at a time) · **2026-08-14 (§17–§31 — the eight objections keeping the decision open with a pre-registered measurement rule, six corrections to earlier claims, and reference material on ZFS internals, the object model, and Ceph across sites)**. Every section heading carries its own date; the footer lists them individually. · **2026-08-15 (§2.5 — correction: panic bug #17659 is fixed in 2.4.x, and Proxmox VE 9 ships 2.4.3, not 2.3.x)**
...
| 6 | Kernel panic when mounting many snapshots | 🟡 **real on 2.3.x, fixed in 2.4.x** | A long lineage (#13131, #13327), latest incarnation #17659; fix PR #17943 is in 2.4.0+, still absent from 2.3.x (incl. 2.3.8) → on 2.3.x the mitigations in §2.5 still apply. |
...
- **The fix:** [PR #17943](https://github.com/openzfs/zfs/pull/17943) (a per-entry mutex) — **merged to master on 8 Dec 2025**.
- ✅ **Added 2026-08-15 — where the fix is and where it isn't.** Verified from **source content**, not from changelogs: searching for the `se_mtx` mutex in `module/os/linux/zfs/zfs_ctldir.c` at individual tags. It **is present in `zfs-2.4.0`, `2.4.1` and `2.4.3`; absent from `zfs-2.3.3` and even from the latest `zfs-2.3.8`** (6/2026). On the 2.3.x line the mitigations therefore still apply; on 2.4.x the panic is gone.
- ⚠️ **Correction to an earlier claim:** the document cited Proxmox VE 9 as an example of a distribution shipping 2.3.x. **It does not.** Reading its repository directly (`pve-no-subscription`, trixie) shows `zfsutils-linux` up to **2.4.3-pve1**. The number had come from an article about the PVE 9.0 release rather than from the repo — a distribution's version has to be read from the `Packages` index.
- 📌 **A methodological note:** an ancestry test (`git compare` of the merge commit against a tag) **does not answer this question** — a cherry-pick into a release branch produces a new SHA, so it reports `diverged` even where the change is present. What decides it is the file content at that tag.
