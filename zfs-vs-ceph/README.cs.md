...
*Opraveno týž den, poté co čtenář upozornil, že tu byl popsaný špatný mechanismus.* Původní text tvrdil, že se nedostupné OSD po `mon_osd_down_out_interval` (default **10 minut**) označí `out` a jeho data se re-replikují na přeživší uzly. **Při třech uzlech, `size=3` a failure doméně na úrovni hostu to nastat nemůže**: každý host už kopii má a CRUSH nemá třetí host, kam by ji umístil. PG prostě zůstanou `undersized+degraded` a **nic se nepřesouvá**. Data by se hnula jen tam, kde umístění existuje — při failure doméně OSD místo hostu (jednouzlový případ, §30.1), nebo při čtyřech a více hostech.

Ta cena je reálná, ale přichází později a z jiného mechanismu. Až se uzel vrátí, jestli dožene levně nebo draze rozhoduje **PG log**, ne to, jak byl označený: pokud log pořád pokrývá zmeškané zápisy, provede Ceph log-based **recovery** úměrnou tomu, co se změnilo. Pokud výpadek přerostl okno logu — ohraničené `osd_min_pg_log_entries` v čistém stavu a `osd_max_pg_log_entries` v nečistém —, musí se PG **backfillovat** celá, a *"lowering the values for both … will cause Ceph to go into backfilling much earlier"*. Backfill je to místo, kde přes linku putují data celého uzlu.
