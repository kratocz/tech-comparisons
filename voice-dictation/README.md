...
- **Adversarial verification:** performed 2026-08-18 in a fresh context with a "refute" brief. No claim fell — including every impossibility claim; one was weakened and is incorporated: building VoiceInk from source unlocks the app only via the official `make local` path, and loses iCloud dictionary sync in addition to updates (§3, §8).
...
- **VoiceInk** — open source (GPL-3.0) with paid binaries: one-time $29 / $49 / $69 by number of Macs, lifetime updates. The README says explicitly: *"As an open-source project, you can build VoiceInk yourself by following the instructions in BUILDING.md."* The paid build adds automatic updates and developer support — building from source is a legal and officially documented path, not a license workaround. A nuance from verifying the sources: the code contains a full license gate with a 7-day trial, and the official `make local` build path deliberately disables it (compile flag `LOCAL_BUILD`) — anyone who compiles the app directly in Xcode without that flag gets the trial version and could wrongly conclude that "free from source" does not hold. Per BUILDING.md the local build also lacks automatic updates and iCloud dictionary sync.
...
- **Building from source = no automatic updates, no iCloud dictionary sync, no developer support.** Updating means fetching and compiling new sources — via the official `make local` path, not directly in Xcode, otherwise the trial gate activates (§3). Anyone who does not want that buys Solo for $29 — still a fraction of Wispr Flow's yearly price.
