...
| Licence / náklady | ✅ CDDL — zdarma (nesmí do binárky jádra) | ✅ LGPL-2.1/3 — zdarma | ✅ LGPL-2.1/3 — zdarma | ✅ GPL-2.0 — zdarma, v jádře | ✅ GPL-3.0 — zdarma | 🟡 AGPL-3.0 — zdarma; validované aktualizace chtějí předplatné (§11, §16) |
...
**Předplatné Proxmox Backup Serveru k 2026-08-13**, na server a rok, všechny varianty s neomezeným úložištěm záloh i počtem klientů: Community €560 (jen komunitní podpora), Basic €1 120 (5 ticketů/rok, 1 pracovní den), Standard €2 240 (15 ticketů/rok, 4 h), Premium €4 480 (neomezeně ticketů, 2 h). Každá placená varianta odemyká enterprise repozitář; rozdíly nad rámec toho jsou v objemu podpory a době odezvy (§16). Samotný software je zdarma ve všech variantách — viz licenční řádek v tabulce.
...
- **Pravidlo 2 — žádné resume.** Ani jedna man page resume nedokumentuje a tracker btrbk zaznamenává přerušené přenosy, které začínají znovu, místo aby navazovaly (níže) — absence doložená ze dvou stran, ne dovozená z jedné mlčící stránky. Přerušený přenos začíná od nuly. Na rezidenční WAN s inkrementy v jednotkách TiB je to samo o sobě diskvalifikace — je to přesně ten failure mode, kvůli kterému pravidlo 2 existuje.
...
**Licence a proč je ta buňka 🟡, ne ✅.** PBS je *"free and open source software … licensed under the GNU Affero General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version"* a nic ze srovnání výše není za paywallem — šifrování na klientu, verify joby, deduplikace i sync joby mezi lokalitami jsou ve verzi zdarma. Předplatné kupuje **enterprise repozitář balíčků** a podporu. Bez něj bereš no-subscription repozitář, o kterém Proxmox píše, že je *"not recommended … on production servers, because these packages are not always heavily tested and validated."* To je tvrzení o kadenci testování balíčků, ne o okleštěném produktu, a pro sólo homelab je to normální cesta — je to ale doporučení výrobce, které zbylých pět sloupců nemá, odtud 🟡 místo ✅. Pro pořádek: totéž rozdělení platí i pro samotný Proxmox VE, takže volbou ZFS na PVE se tomu neuteče; jen to není vlastnost `zfs send`. Aktuální ceny jsou pomíjivé a leží v §11.
...
- Licence (řádek tabulky): [OpenZFS — License (CDDL a proč ZFS jede jako modul)](https://openzfs.github.io/openzfs-docs/License.html), [ceph/ceph `COPYING` — LGPL-2.1 nebo LGPL-3](https://github.com/ceph/ceph/blob/main/COPYING), [Linux `fs/btrfs` — SPDX GPL-2.0](https://github.com/torvalds/linux/blob/master/fs/btrfs/ctree.c), [rsync `COPYING` — GPL-3.0](https://github.com/WayneD/rsync/blob/master/COPYING), [PBS — Introduction (AGPL-3.0)](https://pbs.proxmox.com/docs/introduction.html), [PBS — Installation (no-subscription repozitář)](https://pbs.proxmox.com/docs/installation.html), [Proxmox — ceník PBS](https://www.proxmox.com/en/products/proxmox-backup-server/pricing)
