...
| Rename / move of a large tree (§14) | ✅ metadata only (within a dataset) | ✅ invisible — guest-FS metadata | ❌ **delete + full re-copy** | ✅ explicit `RENAME` command | ❌ delete + re-transfer (`--fuzzy` misses directory renames) | ✅ chunks re-referenced, not re-uploaded |
| **▸ Atomicity and consistency** (§6) | | | | | | |
| Destination is always a valid past state | ✅ transactional `recv` | ✅ delta applies wholly, or rolls back | ❌ live directory is a mix during a sync | 🟡 prior snapshots untouched; the in-flight subvolume is separate | ❌ | ✅ completed snapshots are immutable |
| Consistent point after a mid-transfer crash | ✅ last received snapshot | ✅ last mirror snapshot | 🟡 last **completed** snapshot on the remote | 🟡 last completed (read-only) subvolume; the partial one is left behind | ❌ none | ✅ last completed snapshot |
| Resume after a link outage | ✅ resume token (`recv -s`) | 🟡 the daemon continues; DIY `export-diff` does not | 🟡 the daemon continues (per file) | ❌ **none — restart from zero** | 🟡 `--partial` | 🟡 no token, but a retry sends only missing chunks |
| **▸ Link and budget** | | | | | | |
| **Transfer size known in advance** | ✅ `zfs send -nvP` (exact) | ✅ `rbd diff --format json` (sum the extents) | ❌ no | ❌ no dry-run option | ❌ `--dry-run` gives only a list | ❌ no |
| Compression on the wire | ✅ `-c` ships blocks compressed as they sit on disk | 🟡 external (ssh `-C`) | 🟡 external | ✅ `--compressed-data` (Linux 6.0+) | ✅ `-z` | ✅ zstd per chunk, client-side |
| Transfer without a key on the destination | ✅ `send -w` (raw) | ❌ | ❌ | ❌ no native encryption | ❌ | ✅ **client-side AES-256-GCM** |
| Bandwidth limiting | ✅ zrepl / `pv` / `mbuffer` | 🟡 daemon configuration | 🟡 daemon configuration | ✅ `pv` / `mbuffer` | ✅ `--bwlimit` | 🟡 traffic control — but sync jobs need their own `rate-in` (§16) |
| **▸ Operations** | | | | | | |
| Daemon required | ✅ none (or zrepl) | ❌ `rbd-mirror` | ❌ `cephfs-mirror` | ✅ none (or btrbk) | ✅ none | ❌ a PBS instance on both ends |
| Where the daemon runs / direction | ✅ push or pull | 🟡 **secondary** (pull) | 🟡 **primary** (push) | ✅ push or pull | ✅ either | 🟡 sync job: pull or push |
| Bidirectional / failback | 🟡 manual role swap | ✅ promote/demote, two-way | ❌ one-way, **single peer** | 🟡 manual role swap | 🟡 manual | ❌ **restore, not failover** (§16) |
| Min. nodes on the destination | ✅ **1** | 🟡 1 supported, not production-grade (§13) | 🟡 1, plus a kernel-client caveat (§13) | ✅ **1** | ✅ 1 | ✅ **1** |
| Licence / cost | ✅ CDDL — free (not shippable inside the kernel binary) | ✅ LGPL-2.1/3 — free | ✅ LGPL-2.1/3 — free | ✅ GPL-2.0 — free, in-tree | ✅ GPL-3.0 — free | 🟡 AGPL-3.0 — free; validated updates need a subscription (§11, §16) |
| **▸ Fit for the workload** | | | | | | |
| Large continuously-modified files (VM, DB) | ✅ | ✅ | ❌ transfers the whole file | 🟡 files only — no ZVOL equivalent (§15) | 🟡 delta yes, but reads the whole file | ✅ dirty bitmap while the VM runs |
| Millions of small files, few changes | ✅ | — | ✅ | ✅ | ❌ the walk dominates the transfer | 🟡 metadata mode avoids re-reading |
| In-place rewrite / recompression (§8) | ❌ ships every dirtied block | ❌ ditto | ✅ ships only changed files | ❌ ships every changed extent | ✅ ships only changed content | ✅ content-addressed — identical content dedups away |
| Writable / RWX destination | ❌ destination must be `readonly` | ❌ | ✅ (but should not be) | ❌ received subvolume is read-only | ✅ | ❌ a datastore; you must restore |
| Subset of a dataset / different layout | ❌ whole dataset | ❌ whole image | ✅ per directory | ❌ whole subvolume | ✅ freely | ✅ freely |
| Transfer between different engines | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ **engine-agnostic** |

### How to read it

Read the PBS column against §16 first: it is a backup system, so its ❌ on bidirectional failback is a category statement, not a defect — while its ✅ on client-side encryption, engine independence and rename cost are rows where it genuinely beats every replication mechanism here. Btrfs (§15) is the closest thing to ZFS in the table and the sharpest test of the verdict: same class of mechanism, comparable granularity, and the only column that represents a rename *as* a rename. It loses here on two of the four decision rules — no resume and no size estimate — which is precisely the pair that this context turns on. ZFS wins everywhere the question is "how many bytes will move and what happens when the link drops" — block granularity, an exact estimate up front, a transactional receive and a resume token. RBD is its equal at the block level and beats it on one row (native bidirectional failover with promote/demote), but pays with three nodes on the destination side and a mandatory daemon. CephFS mirror wins only where the others cannot compete at all — a shared writable filesystem and replication per directory rather than per whole dataset — and loses on granularity, atomicity and hardlinks. rsync is the last column not because it is bad, but because it is the only one that does what none of the others can: **change the engine, change the layout, take a subset** — and in one scenario (§8) it beats all of them.
...
**Proxmox Backup Server subscription tiers as of 2026-08-13**, per server per year, all with unlimited backup storage and clients: Community €560 (community support only), Basic €1,120 (5 tickets/year, one business day), Standard €2,240 (15 tickets/year, 4 h), Premium €4,480 (unlimited tickets, 2 h). Every paid tier unlocks the enterprise repository; the differences above that are support volume and response time (§16). The software itself is free at every tier — see the licence row in the table.
...
- **Rule 2 — no resume.** Neither man page documents a resume option, and the btrbk tracker records interrupted transfers restarting rather than resuming (below) — an absence argued from two directions rather than inferred from one silent page. An interrupted transfer starts again from zero. On a residential WAN moving multi-TiB increments, that alone is disqualifying — it is the exact failure mode rule 2 exists to exclude.
...
**Licensing, and why its cell is 🟡 rather than ✅.** PBS is *"free and open source software … licensed under the GNU Affero General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version"*, and nothing in the comparison above sits behind a paywall — client-side encryption, verify jobs, deduplication and cross-site sync jobs are all in the free build. What a subscription buys is the **enterprise package repository** and support. Without one you take the no-subscription repository, which Proxmox documents as *"not recommended … on production servers, because these packages are not always heavily tested and validated."* That is a statement about package-testing cadence, not a crippled product, and for a solo homelab it is the normal path — but it is a vendor recommendation the other five columns do not have, hence 🟡 rather than ✅. In fairness the same split applies to Proxmox VE itself, so choosing ZFS-on-PVE does not escape it; it simply is not a property of `zfs send`. Current prices are perishable and live in §11.
...
- Licensing (table row): [OpenZFS — License (CDDL, and why ZFS ships as a module)](https://openzfs.github.io/openzfs-docs/License.html), [ceph/ceph `COPYING` — LGPL-2.1 or LGPL-3](https://github.com/ceph/ceph/blob/main/COPYING), [Linux `fs/btrfs` — SPDX GPL-2.0](https://github.com/torvalds/linux/blob/master/fs/btrfs/ctree.c), [rsync `COPYING` — GPL-3.0](https://github.com/WayneD/rsync/blob/master/COPYING), [PBS — Introduction (AGPL-3.0)](https://pbs.proxmox.com/docs/introduction.html), [PBS — Installation (no-subscription repository)](https://pbs.proxmox.com/docs/installation.html), [Proxmox — PBS pricing](https://www.proxmox.com/en/products/proxmox-backup-server/pricing)
